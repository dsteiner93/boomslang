...
let add_func_signature map func_signature = SignatureMap.add func_signature (Primitive(Void)) map
in
let func_signatures_for_class classdecl =
  let original_map = (List.fold_left add_fdecl SignatureMap.empty classdecl.methods) in
  List.fold_left add_func_signature original_map (get_autogenerated_constructor_signatures classdecl)
in
...
check_object_instantiation class_name exprs v_symbol_tables =
  ...
  if StringMap.mem class_name class_signatures then
    ...
    if SignatureMap.mem signature signatures_in_class then
      (Class(class_name), SObjectInstantiation(class_name, checked_exprs))
    else raise (Failure("Attempted to initialize class " ^ class_name ^ " using a type signature that has no associated constructor."))
