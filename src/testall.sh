...
Usage() {
    echo "Usage: testall.sh [options] [.boom files]"
...
CheckFail() {
    ...
    generatedfiles="$generatedfiles ${basename}check.err ${basename}.diff" &&
    RunFail "$BOOMC" "<" $1 "2>" "${basename}check.err" ">>" $globallog &&
    Compare ${basename}check.err ${reffile}.err ${basename}.diff
...
then
    ...
    files="tests/test-*.boom tests/fail-*.boom"
fi
